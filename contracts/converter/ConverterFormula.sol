...
contract ConverterFormula is Utils {
    ...
    /**
        @dev given a token supply, connector balance, weight and a deposit amount (in wei),
        calculates the return for a given conversion (in the main token)

        Formula:
        Return = _tokenSupply * ((1 + _depositAmount / _ethBalance) ^ (_ethWeight / 1000000) - 1)

        @param _tokenSupply    token total supply
        @param _ethBalance     total connector balance
        @param _ethWeight      connector weight, represented in ppm, 1-1000000
        @param _depositAmount  deposit amount, in connector token

        @return purchase return amount
    */
    function calculatePurchaseReturn(uint256 _tokenSupply, uint256 _ethBalance, uint32 _ethWeight, uint256 _depositAmount)
    internal view returns (uint256)
    {
        // validate input
        require(_tokenSupply > 0 && _ethWeight > 0 && _ethWeight <= MAX_WEIGHT);
        ...
        if (_ethWeight == MAX_WEIGHT)
            return safeMul(_tokenSupply, _depositAmount) / _ethBalance;

        uint256 result;
        uint8 precision;
        uint256 baseN = safeAdd(_depositAmount, _ethBalance);
        (result, precision) = power(baseN, _ethBalance, _ethWeight, MAX_WEIGHT);
        uint256 temp = safeMul(_tokenSupply, result) >> precision;
        return temp - _tokenSupply;
    }

    /**
        @dev given a token supply, connector balance, weight and a sell amount (in the main token),
        calculates the return in ETH

        Formula:
        Return = _ethBalance * (1 - (1 - _sellAmount / _tokenSupply) ^ (1 / (_ethWeight / 1000000)))

        @param _tokenSupply   token total supply
        @param _ethBalance    total connector
        @param _ethWeight     constant connector Weight, represented in ppm, 1-1000000
        @param _sellAmount    sell amount, in the token itself

        @return sale return amount
    */
    function calculateSaleReturn(uint256 _tokenSupply, uint256 _ethBalance, uint32 _ethWeight, uint256 _sellAmount)
    internal view returns (uint256)
    {
        // validate input
        require(_tokenSupply > 0 && _ethWeight > 0 && _ethWeight <= MAX_WEIGHT);
        ...
        // special case if the weight = 100%
        if (_ethWeight == MAX_WEIGHT)
            return safeMul(_ethBalance, _sellAmount) / _tokenSupply;

        uint256 result;
        uint8 precision;
        uint256 baseD = _tokenSupply - _sellAmount;
        (result, precision) = power(_tokenSupply, baseD, MAX_WEIGHT, _ethWeight);
        uint256 temp1 = safeMul(_ethBalance, result);
        uint256 temp2 = _ethBalance << precision;
